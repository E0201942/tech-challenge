...
module.exports.home = async (req,res,next)=>{    
  ...
  //Get name of the album selected
  ...
  if(token){
    try{
    //Get Temporary Links to Photos  in the selected album
      let paths = await getLinksAsync(token, path); 
    //Get paths to other albums
      let folderpath = await getFolderLinksAsync(token, '');
      path = path.replace("/", "");
      folderarr = {};
      //To keep track of which album is currently seleted
      currfolder ={};
      //Check if current album is the root album
      if(path != ""){
      ...
      //Checks if the album is empty, adds an image saying so if it is
      emptyarr =[];
      if(paths.length == 0){
      ...
      //Adds the names of all available albums as well as their paths
...
//Creates a new folder named New Album
module.exports.createfolder = async (req,res,next)=>{
//Gets the name of the folder to be created (For future work)
...
//Gets the path of the folder to delete
  path = req.url.replace("delete", "")
  path = path.replace("/", "");
  path = path.replace("%20", " ");
  path = path.replace("?", "");
  //check to see if current folder is root folder, do not delete if it is
  if(path.length > 1){
  try{
    let options={
...
//Placeholder to upload a sample impage (Changed in future work)
  var localpath = "public/images/a.jpg";
//Get path of folder to upload to
  ...
  //check if current folder is root folder and add a '/' to the filename if it is not
  if(path.length > 0){
  	filename = "/"+ filename;
  }
  try{
